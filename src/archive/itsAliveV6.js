...
void main() {
  ...
  // Create circular movement for particles with thicker ripples
  float angle = time + uv.y * PI * 2.0;
  vec2 circularMovement = vec2(cos(angle), sin(angle)) * 0.2; // Increased amplitude for thicker ripples
  uv += circularMovement;
  ...
  // Apply circular mask
  vec4 blurTexColor = texture2D(blurTex, vUv);
  float mask = blurTexColor.a;
  vec3 finalColor = mix(back, result, mask);

  gl_FragColor = vec4(finalColor, opacity * blurAlpha);
}
`;
...
function init() {
    ...
    const uniforms = {
        ...
        back: { value: new THREE.Color(0x0D0D0D) },
        style: { value: 0.0 },
        param1: { value: 1.0 },
        param2: { value: 1.0 },
        param3: { value: 1.0 }
