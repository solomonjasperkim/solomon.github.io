...
//         back: { value: new THREE.Color(0xffffff) }, // Change background color to white
//         style: { value: 0.0 },
//         param1: { value: 1.0 },
//         param2: { value: 1.0 },
//         param3: { value: 1.0 }
...
void main() {
  vec2 uv = vUv;
  vec3 grainColor = texture2D(grainTex, uv).rgb;
  float gr = pow(grainColor.r * 1.0, 1.5);
  float gg = grainColor.g;
  float ax = param2 * gr * cos(gg * 2.0 * PI);
  float ay = param2 * gr * sin(gg * 2.0 * PI);
  float nx = uv.x + ax;
  ...
  // Use white color for shadow effect
  float shadowWaveX = 0.5 + 0.5 * sin(time * 0.5 + uv.x * 10.0);
  float shadowWaveY = 0.5 + 0.5 * cos(time * 0.5 + uv.y * 10.0);
  float shadowEffect = shadowWaveX * shadowWaveY;
  vec3 shadowColor = vec3(1.0);

  vec3 front = vec3(1.0); // White sparkles
  vec3 result = mix(back, shadowColor * front, n); // Mix with white background
...
function init() {
    ...
    const uniforms = {
        ...
        resolution: { value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
        grainTex: { value: grainTexture },
        back: { value: new THREE.Color(0xffffff) }, // White background
        param1: { value: 1.0 },
        param2: { value: 1.0 },
        param3: { value: 1.0 }
    };
